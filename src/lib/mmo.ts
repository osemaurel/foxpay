...
/**
 * Les codes opérateurs se terminent par le pays : `MTN_MOMO_CIV`. Ce qui
 * précède identifie l'opérateur, et peut lui-même contenir des soulignés.
 */
...
export function providerName(code: string | null): string | null {
  ...
  const cle = code.replace(/_[A-Z]{3}$/, '')
